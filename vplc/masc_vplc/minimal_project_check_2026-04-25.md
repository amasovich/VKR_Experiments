...
## Примечание аудита 2026-05-02

При локальной инспекции текущего файла `vplc\masc_vplc\vkr_minimal_vplc_test\vkr_minimal_vplc_test.vplc` как SQLite-базы зафиксировано:

```text
schema_version: 4.1.4
version: 1.0.7
task code: cycle_counter := cycle_counter + 1;
```

Это не отменяет результаты bare-metal проверки 2026-04-25, где по скриншотам и CLI snapshot была подтверждена версия программы `1.0.1`. Текущий файл проекта был позднее повторно открыт/собран при контейнерном smoke-test, поэтому версия `1.0.7` фиксируется как состояние проектного файла после последующей функциональной проверки, а не как новый экспериментальный результат или KPI.
